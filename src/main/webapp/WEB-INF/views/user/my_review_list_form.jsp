		...
		<div class="col-xl">
			<div class="row">
				...
				<div class="col" style="position: absolute; left: 90%;">
					<a href="/guest/review/update_form/${review.id}" style="margin-right: 10px;">수정</a> <a onclick="index.deleteReview(${review.id})" style="color: red; margin-right: 10px; cursor: pointer;">삭제</a>
				</div>
...
<div class="container justify-content-center" style="margin-left: 47%">
	<c:if test="${reviewCount ne 0}">
		<ul class="pagination justify-content-center">
			...
			<li class="page-item ${reviews.first ? isDisabled : isAbled}"><a class="page-link" href="/guest/my-review-list/${principal.user.id}?page=${reviews.number - 1}">Prev</a></li>

			<c:forEach var="num" items="${pageNumbers}">
				<c:choose>
					<c:when test="${reviews.number + 1 eq num}">
						<li class="page-item active"><a class="page-link" href="/guest/my-review-list/${principal.user.id}?page=${num - 1}">${num}</a></li>
					</c:when>
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="/guest/my-review-list/${principal.user.id}?page=${num - 1}">${num}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<li class="page-item ${reviews.last ? isDisabled : isAbled}"><a class="page-link" href="/guest/my-review-list/${principal.user.id}?page=${reviews.number + 1}">Next</a></li>
